...
// NewBlockChainRepository is constructor for bookRepository.
func NewBlockChainRepository(logger logger.Logger, conf *config.Config) Repository {
	...
	logger.GetZapLogger().Infof("Success database connection, %s:%s", conf.MySql.Host, conf.MySql.Port)
	return &blockChainRepository{&repository{db: db}}
}
...
func connectDatabase(logger logger.Logger, config *config.Config) (*gorm.DB, error) {
	...
	dsn = fmt.Sprintf("%s:%s@(%s)/%s?charset=utf8&parseTime=True&loc=Local", config.MySql.Username, config.MySql.Password, config.MySql.Host, config.MySql.Dbname)
